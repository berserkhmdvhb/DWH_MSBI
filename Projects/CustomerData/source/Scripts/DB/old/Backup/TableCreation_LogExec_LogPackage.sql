...
CREATE TABLE [log].[Tl_Exec] (
    ...
    CONSTRAINT [PK_Tl_Exec] PRIMARY KEY CLUSTERED ([LogId] ASC),
    CONSTRAINT [FK_Tl_Exec_PackageId] FOREIGN KEY ([PackageId])
        REFERENCES [log].[Tl_Packages] ([PackageId])
        ON DELETE CASCADE ON UPDATE CASCADE
);

-- Optional Index for faster lookups by PackageId and RunTime
CREATE NONCLUSTERED INDEX [IX_Tl_Exec_PackageId_RunTime]
ON [log].[Tl_Exec] ([PackageId], [RunTime]);