...
-- =============================================
ALTER PROCEDURE CustomerInfoFull
...
BEGIN
    ...
    SELECT 
        ...
        Td_St.[StateName]
    FROM [CustomerDWH].[dbo].[FactCustomer] Tf_C
    JOIN [CustomerDWH].[dbo].[DimCountry] AS Td_C ON Tf_C.CountryID_FK = Td_C.CountryID
    JOIN [CustomerDWH].[dbo].[DimDate] AS Td_D ON Td_D.[date_key] = Tf_C.SalesDateID_FK
    JOIN [CustomerDWH].[dbo].[DimProduct] AS Td_P ON Tf_C.ProductID_FK = Td_P.ProductID
    JOIN [CustomerDWH].[dbo].[DimSalesPerson] AS Td_S ON Tf_C.SalesPersonID_FK = Td_S.SalesPersonID
    JOIN [CustomerDWH].[dbo].[DimState] AS Td_St ON Tf_C.StateID_FK = Td_St.StateID
